  ...
  play(){
    ...
    if (playerid === 1 ){
      ...
      if(player.jump <= -100 && (player.distance === 1650 || player.distance === 2450 || player.distance === 3850 || player.distance === 5250 || player.distance === 5650) ){
        playerstate = "out";
        maintxt = "You just crashed into the hurdle. Better luck next time!"
        gameState = 3;
      }
      if(player.jump >= -30 && ( player.distance === 2100||player.distance === 3200) ){
        playerstate = "out";
        maintxt = "It appears you cheating, as you were not jumping. You were floating."
        text("It appears you cheating, as you were not jumping. You were floating.",displayWidth /2, displayHeight/2)
        gameState = 3;
      }
    }
    ...
    restart.mousePressed(()=>{
      retxt = "To restart, reload this page. BOTH players have to do this."
      re.html(retxt)
      database.ref('/').set({
        playerCount: 0,
        gameState : 0
